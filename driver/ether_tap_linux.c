...
ether_tap_open(struct net_device *dev)
{
    ...
    ifr.ifr_flags = IFF_TAP | IFF_NO_PI; /* TAP mode, no packet information */
    ...
    if (memcpy(dev->addr, ETHER_ADDR_ANY, ETHER_ADDR_LEN) == 0) /* when hwaddr is not set */
    {
...
ether_tap_init(const char *name, const char *addr)
{
    ...
    strncpy(tap->name, name, sizeof(tap->name) - 1); /* strings are terminated by a zero, so the length is always plus one */
    tap->fd = -1;                                    /* init */
    dev->priv = tap;
