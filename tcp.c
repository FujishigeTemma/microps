...
tcp_pcb_release(struct tcp_pcb *pcb)
{
  if (pcb->wait)
  {
    pthread_cond_broadcast(&pcb->cond); /* tcp_pcb_release() will be called in other thread */
    return;
  }
  pthread_cond_destroy(&pcb->cond);
  memset(pcb, 0, sizeof(*pcb)); /* TCP_PCB_STATE_FREE = 0 */
}
...
tcp_pcb_select(struct tcp_endpoint *local, struct tcp_endpoint *foreign)
{
  ...
  for (pcb = pcbs; pcb < tailof(pcbs); pcb++)
  {
    ...
    {
      /* NOTE: When called in bind() function, foreign is NULL. */
      if (!foreign)
      {
        return pcb;
      }
...
tcp_pcb_get(int id)
{
  ...
  if (id < 0 || id >= (int)countof(pcbs)) /* out of range */
  {
    return NULL;
  }
...
tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, uint8_t *data, size_t len, struct tcp_endpoint *local, struct tcp_endpoint *foreign)
{
  ...
  debugf("%s => %s, len=%zu (payload=%zu)", tcp_endpoint_ntop(local, ep1, sizeof(ep1)), tcp_endpoint_ntop(foreign, ep2, sizeof(ep2)), total, len);
...
tcp_retransmit_queue_emit(void *arg, void *data)
{
  struct tcp_pcb *pcb;
  struct tcp_queue_entry *entry;
  struct timeval nom, diff, timeout;

  pcb = (struct tcp_pcb *)arg;
  entry = (struct tcp_queue_entry *)data(&pcb->queue);
  gettimeofday(&now, NULL);
  timesub(&now, &entry->first, &diff);
  if (diff.tv_sec >= TCP_PCB_STATE_CLOSED)
  {
    pcb->state = TCP_PCB_STATE_CLOSED;
...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct tcp_endpoint *local, struct tcp_endpoint *foreign)
{
  ...
  if (!pcb || pcb->state == TCP_PCB_STATE_CLOSED) /* when packet arrives at unused port, return RST */
  {
  ...
  */
  if (!TCP_FLG_ISSET(flags, TCP_FLG_ACK)) /* NOTE: After TCP conn ESTABLISHED, all packets dropped without ACK */
  {
    /* drop segment */
    return;
  }
  ...
  case TCP_PCB_STATE_ESTABLISHED:
    if (pcb->snd.una < seg->ack && seg->ack <= pcb->snd.nxt)
    {
      ...
      /* ignore: Users should receive positive acknowledgments for buffers which have been SENT and fully acknowledged (i.e., SEND buffer should be returned with "ok" response) */
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
  ...
  {
    errorf("only supports unicast, src=%s, dst=%s", ip_addr_ntop(src, addr1, sizeof(addr1)), ip_addr_ntop(dst, addr2, sizeof(addr2)));
    return;
  }
...
int tcp_open_rfc793(struct tcp_endpoint *local, struct tcp_endpoint *foreign, int active)
{
  ...
  else
  {
    ...
    pcb->local = *local;
    if (foreign) /* RFC793 says you can restrict foreign address to listen */
    {
      pcb->foreign = *foreign;
    }
...
int tcp_close(int id)
{
  ...
  tcp_output(pcb, TCP_FLG_RST, NULL, 0); /* force disconnection */
  tcp_pcb_release(pcb);
  pthread_mutex_unlock(&mutex);
...
RETRY:
  ...
  case TCP_PCB_STATE_ESTABLISHED:
    ...
    /* NOTE: To prevent fragmentation from occurring at the IP layer (if fragmented, the impact of packet loss is too great). */
