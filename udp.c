...
/* NOTE: the data follows right after this structure */
...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
  ...
  psum = ~cksum16((uint16_t *)&pseudo, sizeof(pseudo), 0); /* bit reinversion */
...
int udp_bind(int id, struct udp_endpoint *local)
{
  ...
  if (exist)
  {
    errorf("already in use, id=%d, want=%s, exist=%s", id, udp_endpoint_ntop(local, ep1, sizeof(ep1)), udp_endpoint_ntop(&exist->local, ep2, sizeof(ep2)));
    pthread_mutex_unlock(&mutex);
    return -1;
  }
  pcb->local = *local;
