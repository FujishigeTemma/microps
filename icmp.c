...
int icmp_output(uint8_t type, uint8_t code, uint32_t values, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
  ...
  hdr->code = code;
  hdr->sum = 0;
  hdr->values = values; /* already network byte order */
  memcpy(hdr + 1, data, len);

  total = sizeof(*hdr) + len;
  hdr->sum = cksum16((uint16_t *)hdr, total, 0);

  debugf("%s => %s, len=%zu", ip_addr_ntop(src, addr1, sizeof(addr1)), ip_addr_ntop(dst, addr2, sizeof(addr2)), total);
  icmp_dump((uint8_t *)hdr, total);

  return ip_output(IP_PROTOCOL_ICMP, (uint8_t *)hdr, total, src, dst);
}
